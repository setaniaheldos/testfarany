...
import { 
  ...
  Star, Download, Mail, Phone, IdCard,
...
export default function Praticiens() {
  ...
  const handleSubmit = (e) => {
    ...
    const url = `https://heldosseva.duckdns.org/praticiens${isEditing ? '/' + formData.cinPraticien : ''}`;
  ...
  const handleDelete = (cinPraticien) => {
    if (window.confirm("⚠️ ATTENTION : La suppression est définitive. Voulez-vous vraiment continuer ?")) {
      axios.delete(`https://heldosseva.duckdns.org/praticiens/${cinPraticien}`)
        .then(() => {
          fetchPraticiens();
  ...
  const deleteSelected = () => {
    ...
    if (window.confirm(`⚠️ Supprimer ${selectedPraticiens.length} praticien(s) sélectionné(s) ?`)) {
      const deletePromises = selectedPraticiens.map(cin => 
        axios.delete(`https://heldosseva.duckdns.org/praticiens/${cin}`)
      );
  ...
  return (
    <div className={`p-4 min-h-screen max-w-[1800px] mx-auto ${containerClasses}`}>
      ...
      <div className={`${cardClasses} p-6 mb-6`}>
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-4 gap-4">
          ...
          <div className="flex flex-wrap gap-3 w-full lg:w-auto">
            <input type="text" placeholder="Nom" className={`${inputClasses} w-full sm:w-36`} value={search.nom} name="nom" onChange={handleSearchChange} />
            <input type="text" placeholder="Prénom" className={`${inputClasses} w-full sm:w-36`} value={search.prenom} name="prenom" onChange={handleSearchChange} />
            <input type="text" placeholder="Spécialité" className={`${inputClasses} w-full sm:w-36`} value={search.specialite} name="specialite" onChange={handleSearchChange} />
            <input type="text" placeholder="Email" className={`${inputClasses} w-full sm:w-36`} value={search.email} name="email" onChange={handleSearchChange} />
          </div>
        </div>
        
        <div className="border-t pt-4 mb-4 transition-colors duration-300" style={{ borderColor: darkMode ? '#374151' : '#e5e7eb' }}>
          <button onClick={() => setShowFilters(!showFilters)} className={`flex items-center gap-2 font-medium transition-colors ${darkMode ? 'text-blue-400 hover:text-blue-300' : 'text-sky-600 hover:text-sky-700'}`}>
          ...
          {showFilters && (
            <div className={`grid grid-cols-1 md:grid-cols-2 gap-4 p-4 rounded-xl mt-4 transition-colors duration-300 ${darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
              <div>
                <label className={`block text-sm font-medium mb-1 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Téléphone</label>
                <input type="text" placeholder="Rechercher par téléphone" className={inputClasses} value={search.telephone} name="telephone" onChange={handleSearchChange} />
              </div>
              
              <div className="flex items-end">
                <button onClick={clearFilters} className={`px-4 py-2 rounded-xl transition-colors flex items-center gap-2 ${darkMode ? 'bg-gray-600 text-white hover:bg-gray-500' : 'bg-gray-500 text-white hover:bg-gray-600'}`}>
                  <X className="w-4 h-4" />
                  Effacer les filtres
                </button>
              </div>
        ...
        <div className="flex flex-wrap gap-3 justify-between">
          <div className="flex flex-wrap gap-3">
            <button onClick={handleAddClick} className={buttonPrimary}>
            ...
            {selectedPraticiens.length > 0 && (
              <button onClick={deleteSelected} className={`flex items-center font-semibold px-6 py-2.5 rounded-xl transition-all shadow-lg transform hover:scale-105 ${darkMode ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-gradient-to-r from-red-500 to-pink-500 hover:from-red-600 hover:to-pink-600 text-white'}`}>
          ...
          <div className="flex flex-wrap gap-3">
            <button onClick={() => setShowStats(!showStats)} className={`flex items-center px-4 py-2.5 rounded-xl transition-all ${darkMode ? 'bg-gray-700 hover:bg-gray-600 text-white' : 'bg-gray-200 hover:bg-gray-300 text-gray-700'}`}>
              {showStats ? <EyeOff className="w-5 h-5 mr-2" /> : <BarChart3 className="w-5 h-5 mr-2" />}
              {showStats ? 'Masquer stats' : 'Afficher stats'}
            </button>
            
            <button onClick={handleExportExcel} className={`flex items-center font-semibold px-6 py-2.5 rounded-xl transition-all shadow-lg transform hover:scale-105 ${darkMode ? 'bg-emerald-600 hover:bg-emerald-700 text-white' : 'bg-gradient-to-r from-emerald-500 to-green-500 hover:from-emerald-600 hover:to-green-600 text-white'}`}>
              <Download className="w-5 h-5 mr-2" /> Export Excel
            </button>
            
            <button onClick={fetchPraticiens} className={`flex items-center px-4 py-2.5 rounded-xl transition-all ${darkMode ? 'bg-indigo-600 hover:bg-indigo-700 text-white' : 'bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white'}`}>
        ...
        <form onSubmit={handleSubmit} className={`${cardClasses} p-8 mb-8 space-y-6`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="flex flex-col">
              ...
              <input className={inputClasses} name="cinPraticien" placeholder="CIN" value={formData.cinPraticien} onChange={handleChange} required disabled={isEditing} />
            </div>

            <div className="flex flex-col">
              <label className={`text-sm font-medium mb-1 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>Nom*</label>
              <input className={inputClasses} name="nom" placeholder="Nom" value={formData.nom} onChange={handleChange} required />
            </div>
            
            <div className="flex flex-col">
              <label className={`text-sm font-medium mb-1 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>Prénom*</label>
              <input className={inputClasses} name="prenom" placeholder="Prénom" value={formData.prenom} onChange={handleChange} required />
            </div>
            ...
            <div className="flex flex-col">
              ...
              <input className={inputClasses} name="telephone" placeholder="Téléphone" value={formData.telephone} onChange={handleChange} />
            </div>
            ...
            <div className="flex flex-col">
              ...
              <input className={inputClasses} name="email" type="email" placeholder="Email" value={formData.email} onChange={handleChange} />
            </div>
            ...
            <div className="flex flex-col">
              ...
              <select className={inputClasses} name="specialite" value={formData.specialite} onChange={handleChange} required>
                <option value="" disabled>Sélectionner la spécialité</option>
          ...
          <div className="flex gap-3 justify-end pt-4">
            ...
            <button type="button" onClick={handleCancel} className={`flex items-center font-semibold px-6 py-2 rounded-xl transition-all shadow-md ${darkMode ? 'bg-gray-600 hover:bg-gray-500 text-white' : 'bg-gray-400 hover:bg-gray-500 text-white'}`}>
      ...
      {/* Tableau des Praticiens (Desktop) */}
      <div className="hidden md:block mt-8">
        <div className={`${cardClasses} overflow-x-auto`}>
          <table className="min-w-[1200px] w-full text-sm">
            <thead>
              <tr className={tableHeaderClasses}>
                <th className="px-4 py-3 text-center w-12">
                  <input type="checkbox" checked={selectedPraticiens.length === filteredPraticiens.length && filteredPraticiens.length > 0} onChange={selectAllPraticiens} className="rounded border-gray-300" />
                </th>
            ...
            <tbody className={darkMode ? 'text-gray-200' : 'text-gray-700'}>
              {loading ? (
                <tr>
                  <td colSpan={8} className="text-center py-8 font-semibold flex items-center justify-center gap-2">
                ...
                paginatedPraticiens.map((p, index) => (
                  <tr key={p.cinPraticien} className={`border-t transition-all duration-200 hover:bg-opacity-50 ${
                    ...
                    <td className="px-4 py-3 text-center">
                      <input type="checkbox" checked={selectedPraticiens.includes(p.cinPraticien)} onChange={() => toggleSelectPraticien(p.cinPraticien)} className="rounded border-gray-300" />
                    </td>
                    ...
                    <td className="px-4 py-3 space-x-2 flex justify-center">
                      <button onClick={() => handleEdit(p)} className={`flex items-center px-3 py-2 rounded-lg transition-all shadow-md transform hover:scale-105 ${darkMode ? 'bg-yellow-600 hover:bg-yellow-700 text-white' : 'bg-gradient-to-r from-yellow-500 to-amber-500 hover:from-yellow-600 hover:to-amber-600 text-white'}`}>
                        <Edit2 className="w-4 h-4 mr-1" /> Modifier
                      </button>
                      <button onClick={() => handleDelete(p.cinPraticien)} className={`flex items-center px-3 py-2 rounded-lg transition-all shadow-md transform hover:scale-105 ${darkMode ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-gradient-to-r from-red-500 to-pink-500 hover:from-red-600 hover:to-pink-600 text-white'}`}>
        ...
        <div className={`mt-4 p-4 rounded-xl text-sm transition-colors duration-300 ${darkMode ? 'bg-gray-800 text-gray-300' : 'bg-blue-50 text-blue-700'}`}>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div className="flex items-center gap-4">
              <span><strong>{filteredPraticiens.length}</strong> praticien(s) trouvé(s)</span>
              {selectedPraticiens.length > 0 && (
                <span className={`px-2 py-1 rounded-full text-xs ${darkMode ? 'bg-blue-600 text-white' : 'bg-blue-100 text-blue-800'}`}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-6">
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className={`px-4 py-2 rounded-xl transition-all font-medium shadow-lg transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed ${darkMode ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-gradient-to-r from-sky-500 to-blue-500 hover:from-sky-600 hover:to-blue-600 text-white'}`}>Précédent</button>
          <span className={`font-bold px-4 py-2 rounded-xl shadow-lg border ${darkMode ? 'bg-gray-800 text-white border-gray-700' : 'bg-white text-sky-800 border-sky-200'}`}>
            Page {page} sur {totalPages || 1}
          </span>
          <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages || totalPages === 0} className={`px-4 py-2 rounded-xl transition-all font-medium shadow-lg transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed ${darkMode ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-gradient-to-r from-sky-500 to-blue-500 hover:from-sky-600 hover:to-blue-600 text-white'}`}>Suivant</button>
        </div>
      </div>
      ...
      <div className="md:hidden grid grid-cols-1 gap-4 mt-6">
        {loading ? (
          <div className={`text-center py-8 font-semibold flex items-center justify-center gap-2 ${darkMode ? 'text-blue-400' : 'text-sky-500'}`}>
            ...
            <div key={p.cinPraticien} className={`${cardClasses} p-5 hover:shadow-xl transition-all duration-200`}>
              <div className="flex items-center justify-between mb-3 pb-3 border-b" style={{ borderColor: darkMode ? '#374151' : '#e5e7eb' }}>
                <div>
                  ...
                  <span className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>CIN: {p.cinPraticien}</span>
                </div>
              ...
              <div className="flex gap-3 mt-4 justify-end">
                <button onClick={() => handleEdit(p)} className={`flex items-center px-4 py-2 rounded-lg transition-all shadow-md transform hover:scale-105 ${darkMode ? 'bg-yellow-600 hover:bg-yellow-700 text-white' : 'bg-gradient-to-r from-yellow-500 to-amber-500 hover:from-yellow-600 hover:to-amber-600 text-white'}`}>
                  <Edit2 className="w-4 h-4 mr-1" /> Modifier
                </button>
                <button onClick={() => handleDelete(p.cinPraticien)} className={`flex items-center px-4 py-2 rounded-lg transition-all shadow-md transform hover:scale-105 ${darkMode ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-gradient-to-r from-red-500 to-pink-500 hover:from-red-600 hover:to-pink-600 text-white'}`}>
                  <Trash2 className="w-4 h-4 mr-1" /> Supprimer
                </button>
              </div>
            </div>
